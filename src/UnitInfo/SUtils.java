...
public class SUtils {
    ...
    public static String floatFormat(float number){
        if(number >= 1000) return UI.formatAmount((int)number);
    ...
    public static float unitRange(UnitType u) {
        ...
        u.weapons.each(w -> w.bullet != null, w -> {
                mrng[0] = Math.max(mrng[0], bulletRange(w.bullet));
    ...
    public static void outvoke(Object ut, String fieldName, Object value) throws IllegalAccessException, NoSuchFieldException {
        Field field = ut.getClass().getDeclaredField(fieldName);
        field.setAccessible(true);
        field.set(ut, value);
    }
