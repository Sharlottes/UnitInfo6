...
public class BarInfo {
    ...
    public static <T extends Teamc> void getInfo(T target) throws IllegalAccessException, NoSuchFieldException {
        ...
        if(target instanceof Turret.TurretBuild turret){
            ...
            strings.set(1, bundle.format("shar-stat.progress", Strings.fixed(Mathf.clamp(charge/((MendProjector)mend.block).reload) * 100f, 2)));
        ...
        if(target instanceof ItemTurret.ItemTurretBuild turret) {
            ...
            else if(target instanceof StorageBlock.StorageBuild sb && !sb.canPickup() && SUtils.invoke(sb, "linkedCore") instanceof CoreBlock.CoreBuild cb){
        ...
        if(target instanceof Unit unit && unit.type != null) {
            ...
            strings.set(3, bundle.format("shar-stat.attr", (int)((block.baseEfficiency + Math.min(block.maxHeatBoost, block.boostScale * block.sumAttribute(block.attribute, crafter.tileX(), crafter.tileY()))) * 100f)));
            colors.set(3, Pal.ammo);
            numbers.set(3, block.boostScale * crafter.attrsum / block.maxHeatBoost);
        }
